...
import mostCategories from "data/most-categories";
  ...
  return (
    <>
      <section className="mb-8">
        ...
        {prev && <button className="hover:scale-[1.06] w-12 absolute text-black -left-6 h-12 z-10 -translate-y-1/2  top-1/2 rounded-full bg-white flex items-center justify-center" onClick={slideFavoritesPrev}><Icon name="prev" size={24}/></button>}
        {next && <button className="hover:scale-[1.06] w-12 absolute text-black -right-6 h-12 z-10 -translate-y-1/2 top-1/2  rounded-full z-10  bg-white flex items-center justify-center" onClick={slideFavoritesNext}><Icon name="next" size={24}/></button>}
        <ScrollContainer
